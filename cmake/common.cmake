...
function(NBL_CREATE_NSC_COMPILE_RULES)
	...
	set(NBL_IMPL_NSC_PREPROCESS_SENTINEL_BEGIN_NAME "__NBL_IMPL_INTERNAL_PREPROCESS_MODE_INCLUDE_FRAGMENT_BEGIN_SENTINEL_BANNED_FOR_CONSUMERS__")
	set(NBL_IMPL_NSC_PREPROCESS_SENTINEL_END_NAME "__NBL_IMPL_INTERNAL_PREPROCESS_MODE_INCLUDE_FRAGMENT_END_SENTINEL_BANNED_FOR_CONSUMERS__")
	set(NBL_IMPL_NSC_PREPROCESS_SENTINEL_BEGIN "struct ${NBL_IMPL_NSC_PREPROCESS_SENTINEL_BEGIN_NAME} {};")
	set(NBL_IMPL_NSC_PREPROCESS_SENTINEL_END "struct ${NBL_IMPL_NSC_PREPROCESS_SENTINEL_END_NAME} {};")
	set(NBL_IMPL_NSC_PREPROCESS_EXTRACT_SCRIPT "${CMAKE_BINARY_DIR}/CMakeFiles/NBLImplExtractPreprocessFragment.cmake")
	set(NBL_IMPL_NSC_PREPROCESS_EXTRACT_SCRIPT_TEMPLATE [=[
...
set(BEGIN_REGEX "struct[ \t\r\n]+@NBL_IMPL_NSC_PREPROCESS_SENTINEL_BEGIN_NAME@[ \t\r\n]*\\{\\}[ \t\r\n]*;")
set(END_REGEX "struct[ \t\r\n]+@NBL_IMPL_NSC_PREPROCESS_SENTINEL_END_NAME@[ \t\r\n]*\\{\\}[ \t\r\n]*;")

string(REGEX MATCH "${BEGIN_REGEX}" BEGIN_MATCH "${INPUT_CONTENT}")
if(BEGIN_MATCH STREQUAL "")
	message(FATAL_ERROR "Preprocess begin sentinel not found in ${INPUT}")
endif()

string(FIND "${INPUT_CONTENT}" "${BEGIN_MATCH}" BEGIN_OFFSET)
string(LENGTH "${BEGIN_MATCH}" BEGIN_LENGTH)
math(EXPR CONTENT_OFFSET "${BEGIN_OFFSET}+${BEGIN_LENGTH}")
string(SUBSTRING "${INPUT_CONTENT}" "${CONTENT_OFFSET}" -1 REMAINDER_CONTENT)

string(REGEX MATCH "${END_REGEX}" END_MATCH "${REMAINDER_CONTENT}")
if(END_MATCH STREQUAL "")
	message(FATAL_ERROR "Preprocess end sentinel not found in ${INPUT}")
endif()

string(FIND "${REMAINDER_CONTENT}" "${END_MATCH}" END_OFFSET)
string(SUBSTRING "${REMAINDER_CONTENT}" 0 "${END_OFFSET}" OUTPUT_CONTENT)
...
file(WRITE "${OUTPUT}" "${OUTPUT_CONTENT}")
	...
	string(CONFIGURE "${NBL_IMPL_NSC_PREPROCESS_EXTRACT_SCRIPT_TEMPLATE}" NBL_IMPL_NSC_PREPROCESS_EXTRACT_SCRIPT_CONTENT @ONLY)
	file(WRITE "${NBL_IMPL_NSC_PREPROCESS_EXTRACT_SCRIPT}" "${NBL_IMPL_NSC_PREPROCESS_EXTRACT_SCRIPT_CONTENT}")
	set(NBL_IMPL_NSC_PREPROCESS_INCLUDE_VIEW [=[
@NBL_IMPL_NSC_PREPROCESS_SENTINEL_BEGIN@
#include "@TARGET_INPUT@"
@NBL_IMPL_NSC_PREPROCESS_SENTINEL_END@
]=])
...
// <- @COMMENT@!
	...
	set(REQUIRED_COMPILE_ONLY_OPTIONS
	...
	if(NSC_DEBUG_EDIF_FILE_BIT)
    	list(APPEND REQUIRED_COMPILE_ONLY_OPTIONS $<$<CONFIG:Debug>:-fspv-debug=file>)
	endif()
	
	if(NSC_DEBUG_EDIF_SOURCE_BIT)
	    list(APPEND REQUIRED_COMPILE_ONLY_OPTIONS $<$<CONFIG:Debug>:-fspv-debug=source>)
	endif()
	
	if(NSC_DEBUG_EDIF_LINE_BIT)
	    list(APPEND REQUIRED_COMPILE_ONLY_OPTIONS $<$<CONFIG:Debug>:-fspv-debug=line>)
	endif()
	
	if(NSC_DEBUG_EDIF_TOOL_BIT)
	    list(APPEND REQUIRED_COMPILE_ONLY_OPTIONS $<$<CONFIG:Debug>:-fspv-debug=tool>)
	endif()
	
	if(NSC_DEBUG_EDIF_NON_SEMANTIC_BIT)
	    list(APPEND REQUIRED_COMPILE_ONLY_OPTIONS $<$<CONFIG:Debug>:-fspv-debug=vulkan-with-source>)
	endif()

	if(NOT NBL_EMBED_BUILTIN_RESOURCES)
		list(APPEND REQUIRED_COMMON_OPTIONS
			-no-nbl-builtins
	...
	if(NOT TARGET ${IMPL_TARGET})
		...
		set_target_properties(${IMPL_TARGET} PROPERTIES VS_GLOBAL_BuildInParallel true)
	endif()
...
namespace @IMPL_NAMESPACE@ {
	...
	set(ALL_OUTPUT_KEYS "")
	set(IMPL_RULE_GROUP_NAMES "")
	set(IMPL_RULE_GROUP_TARGETS "")
	set_target_properties(${IMPL_TARGET} PROPERTIES
		NBL_IMPL_RULE_OUTPUTS ""
		NBL_IMPL_RULE_OUTPUT_TARGETS ""
	)

	set(NBL_IMPL_SPLIT_RULES_BY_GROUP FALSE)
	set(NBL_IMPL_SPLIT_GROUP_NAMES "")
	set(NBL_IMPL_SPLIT_GROUP_IDENTS "")
	set(NBL_IMPL_SPLIT_GROUP_TARGETS "")
	set(NBL_IMPL_SPLIT_UNGROUPED_NAME "__nbl_impl_ungrouped_rules__")
	foreach(INDEX RANGE ${LAST_INDEX})
		string(JSON NBL_IMPL_SPLIT_ITEM_JSON GET "${IMPL_INPUTS}" ${INDEX})
		set(NBL_IMPL_SPLIT_GROUP_NAME "")
		string(JSON NBL_IMPL_SPLIT_GROUP_TYPE ERROR_VARIABLE NBL_IMPL_SPLIT_GROUP_ERROR TYPE "${IMPL_INPUTS}" ${INDEX} GROUP_BY)
		if(NOT NBL_IMPL_SPLIT_GROUP_ERROR)
			set(NBL_IMPL_SPLIT_RULES_BY_GROUP TRUE)
			if(NOT NBL_IMPL_SPLIT_GROUP_TYPE STREQUAL "STRING")
				message(FATAL_ERROR "GROUP_BY must be a string when present")
			endif()
			string(JSON NBL_IMPL_SPLIT_GROUP_NAME GET "${IMPL_INPUTS}" ${INDEX} GROUP_BY)
			string(JSON NBL_IMPL_SPLIT_ITEM_JSON REMOVE "${NBL_IMPL_SPLIT_ITEM_JSON}" GROUP_BY)
		endif()

		set(NBL_IMPL_SPLIT_BUCKET_NAME "${NBL_IMPL_SPLIT_GROUP_NAME}")
		if("${NBL_IMPL_SPLIT_BUCKET_NAME}" STREQUAL "")
			set(NBL_IMPL_SPLIT_BUCKET_NAME "${NBL_IMPL_SPLIT_UNGROUPED_NAME}")
		endif()

		string(MAKE_C_IDENTIFIER "${NBL_IMPL_SPLIT_BUCKET_NAME}" NBL_IMPL_SPLIT_BUCKET_IDENT)
		list(FIND NBL_IMPL_SPLIT_GROUP_IDENTS "${NBL_IMPL_SPLIT_BUCKET_IDENT}" NBL_IMPL_SPLIT_GROUP_INDEX)
		if(NBL_IMPL_SPLIT_GROUP_INDEX EQUAL -1)
			list(APPEND NBL_IMPL_SPLIT_GROUP_NAMES "${NBL_IMPL_SPLIT_BUCKET_NAME}")
			list(APPEND NBL_IMPL_SPLIT_GROUP_IDENTS "${NBL_IMPL_SPLIT_BUCKET_IDENT}")
			set("NBL_IMPL_SPLIT_GROUP_RULES_${NBL_IMPL_SPLIT_BUCKET_IDENT}" "")
		endif()
		list(APPEND "NBL_IMPL_SPLIT_GROUP_RULES_${NBL_IMPL_SPLIT_BUCKET_IDENT}" "${NBL_IMPL_SPLIT_ITEM_JSON}")
	endforeach()

	if(NBL_IMPL_SPLIT_RULES_BY_GROUP)
		get_filename_component(NBL_IMPL_AGGREGATE_INCLUDE_DIR "${IMPL_INCLUDE}" DIRECTORY)
		get_filename_component(NBL_IMPL_AGGREGATE_INCLUDE_NAME "${IMPL_INCLUDE}" NAME_WE)
		get_filename_component(NBL_IMPL_AGGREGATE_INCLUDE_EXT "${IMPL_INCLUDE}" EXT)

		list(LENGTH NBL_IMPL_SPLIT_GROUP_IDENTS NBL_IMPL_SPLIT_GROUP_COUNT)
		if(NBL_IMPL_SPLIT_GROUP_COUNT GREATER 0)
			math(EXPR NBL_IMPL_SPLIT_LAST_GROUP_INDEX "${NBL_IMPL_SPLIT_GROUP_COUNT} - 1")
			foreach(NBL_IMPL_SPLIT_GROUP_INDEX RANGE ${NBL_IMPL_SPLIT_LAST_GROUP_INDEX})
				list(GET NBL_IMPL_SPLIT_GROUP_NAMES ${NBL_IMPL_SPLIT_GROUP_INDEX} NBL_IMPL_SPLIT_BUCKET_NAME)
				list(GET NBL_IMPL_SPLIT_GROUP_IDENTS ${NBL_IMPL_SPLIT_GROUP_INDEX} NBL_IMPL_SPLIT_BUCKET_IDENT)

				set(NBL_IMPL_SPLIT_GROUP_TARGET "${IMPL_TARGET}__${NBL_IMPL_SPLIT_BUCKET_IDENT}")
				if("${NBL_IMPL_AGGREGATE_INCLUDE_DIR}" STREQUAL "")
					set(NBL_IMPL_SPLIT_GROUP_INCLUDE "internal/${NBL_IMPL_AGGREGATE_INCLUDE_NAME}.${NBL_IMPL_SPLIT_BUCKET_IDENT}${NBL_IMPL_AGGREGATE_INCLUDE_EXT}")
				else()
					set(NBL_IMPL_SPLIT_GROUP_INCLUDE "${NBL_IMPL_AGGREGATE_INCLUDE_DIR}/internal/${NBL_IMPL_AGGREGATE_INCLUDE_NAME}.${NBL_IMPL_SPLIT_BUCKET_IDENT}${NBL_IMPL_AGGREGATE_INCLUDE_EXT}")
				endif()

				string(JOIN ",\n" NBL_IMPL_SPLIT_GROUP_RULES_BODY ${NBL_IMPL_SPLIT_GROUP_RULES_${NBL_IMPL_SPLIT_BUCKET_IDENT}})
				set(NBL_IMPL_SPLIT_GROUP_JSON "[\n${NBL_IMPL_SPLIT_GROUP_RULES_BODY}\n]")
				set(NBL_IMPL_SPLIT_GROUP_KEYS "")
				set("NBL_IMPL_SPLIT_GROUP_JSON_${NBL_IMPL_SPLIT_BUCKET_IDENT}" "${NBL_IMPL_SPLIT_GROUP_JSON}")
				list(APPEND NBL_IMPL_SPLIT_GROUP_TARGETS "${NBL_IMPL_SPLIT_GROUP_TARGET}")
				NBL_CREATE_NSC_COMPILE_RULES(
					DISCARD_DEFAULT_GLOB
					TARGET ${NBL_IMPL_SPLIT_GROUP_TARGET}
					BINARY_DIR ${IMPL_BINARY_DIR}
					MOUNT_POINT_DEFINE ${IMPL_MOUNT_POINT_DEFINE}
					COMMON_OPTIONS ${IMPL_COMMON_OPTIONS}
					OUTPUT_VAR NBL_IMPL_SPLIT_GROUP_KEYS
					INCLUDE ${NBL_IMPL_SPLIT_GROUP_INCLUDE}
					NAMESPACE ${IMPL_NAMESPACE}
					DEPENDS ${IMPL_DEPENDS}
					INPUTS ${NBL_IMPL_SPLIT_GROUP_JSON}
				)
				target_link_libraries(${IMPL_TARGET} INTERFACE ${NBL_IMPL_SPLIT_GROUP_TARGET})
				add_dependencies(${IMPL_TARGET} ${NBL_IMPL_SPLIT_GROUP_TARGET})
				list(APPEND ALL_OUTPUT_KEYS ${NBL_IMPL_SPLIT_GROUP_KEYS})
				get_target_property(NBL_IMPL_SPLIT_GROUP_OUTPUTS ${NBL_IMPL_SPLIT_GROUP_TARGET} NBL_IMPL_RULE_OUTPUTS)
				if(NBL_IMPL_SPLIT_GROUP_OUTPUTS)
					foreach(NBL_IMPL_SPLIT_GROUP_OUTPUT IN LISTS NBL_IMPL_SPLIT_GROUP_OUTPUTS)
						NBL_IMPL_REGISTER_RULE_OUTPUT("${NBL_IMPL_SPLIT_GROUP_OUTPUT}" "${NBL_IMPL_SPLIT_GROUP_TARGET}")
					endforeach()
				endif()
				set(HEADER_ITEM_VIEW [=[
#include "@NBL_IMPL_SPLIT_GROUP_INCLUDE@"
			...
			foreach(NBL_IMPL_SPLIT_GROUP_INDEX RANGE ${NBL_IMPL_SPLIT_LAST_GROUP_INDEX})
				list(GET NBL_IMPL_SPLIT_GROUP_NAMES ${NBL_IMPL_SPLIT_GROUP_INDEX} NBL_IMPL_SPLIT_BUCKET_NAME)
				list(GET NBL_IMPL_SPLIT_GROUP_IDENTS ${NBL_IMPL_SPLIT_GROUP_INDEX} NBL_IMPL_SPLIT_BUCKET_IDENT)
				list(GET NBL_IMPL_SPLIT_GROUP_TARGETS ${NBL_IMPL_SPLIT_GROUP_INDEX} NBL_IMPL_SPLIT_GROUP_TARGET)
				set(NBL_IMPL_SPLIT_GROUP_JSON "${NBL_IMPL_SPLIT_GROUP_JSON_${NBL_IMPL_SPLIT_BUCKET_IDENT}}")
				string(JSON NBL_IMPL_SPLIT_GROUP_RULE_COUNT LENGTH "${NBL_IMPL_SPLIT_GROUP_JSON}")
				if(NBL_IMPL_SPLIT_GROUP_RULE_COUNT EQUAL 0)
					continue()
				endif()
				math(EXPR NBL_IMPL_SPLIT_GROUP_LAST_RULE_INDEX "${NBL_IMPL_SPLIT_GROUP_RULE_COUNT} - 1")
				foreach(NBL_IMPL_SPLIT_GROUP_RULE_INDEX RANGE ${NBL_IMPL_SPLIT_GROUP_LAST_RULE_INDEX})
					string(JSON NBL_IMPL_SPLIT_RULE_DEPENDS_TYPE ERROR_VARIABLE NBL_IMPL_SPLIT_RULE_DEPENDS_ERROR TYPE "${NBL_IMPL_SPLIT_GROUP_JSON}" ${NBL_IMPL_SPLIT_GROUP_RULE_INDEX} DEPENDS)
					if(NBL_IMPL_SPLIT_RULE_DEPENDS_ERROR OR NOT NBL_IMPL_SPLIT_RULE_DEPENDS_TYPE STREQUAL "ARRAY")
						continue()
					endif()
					string(JSON NBL_IMPL_SPLIT_RULE_DEPENDS_LENGTH LENGTH "${NBL_IMPL_SPLIT_GROUP_JSON}" ${NBL_IMPL_SPLIT_GROUP_RULE_INDEX} DEPENDS)
					if(NBL_IMPL_SPLIT_RULE_DEPENDS_LENGTH EQUAL 0)
						continue()
					endif()
					math(EXPR NBL_IMPL_SPLIT_RULE_LAST_DEP_INDEX "${NBL_IMPL_SPLIT_RULE_DEPENDS_LENGTH} - 1")
					foreach(NBL_IMPL_SPLIT_RULE_DEP_INDEX RANGE ${NBL_IMPL_SPLIT_RULE_LAST_DEP_INDEX})
						string(JSON NBL_IMPL_SPLIT_RULE_DEP GET "${NBL_IMPL_SPLIT_GROUP_JSON}" ${NBL_IMPL_SPLIT_GROUP_RULE_INDEX} DEPENDS ${NBL_IMPL_SPLIT_RULE_DEP_INDEX})
						get_target_property(NBL_IMPL_SPLIT_ALL_RULE_OUTPUTS ${IMPL_TARGET} NBL_IMPL_RULE_OUTPUTS)
						get_target_property(NBL_IMPL_SPLIT_ALL_RULE_OUTPUT_TARGETS ${IMPL_TARGET} NBL_IMPL_RULE_OUTPUT_TARGETS)
						if(NOT NBL_IMPL_SPLIT_ALL_RULE_OUTPUTS OR NOT NBL_IMPL_SPLIT_ALL_RULE_OUTPUT_TARGETS)
							continue()
						endif()
						list(FIND NBL_IMPL_SPLIT_ALL_RULE_OUTPUTS "${NBL_IMPL_SPLIT_RULE_DEP}" NBL_IMPL_SPLIT_RULE_DEP_OUTPUT_INDEX)
						if(NBL_IMPL_SPLIT_RULE_DEP_OUTPUT_INDEX EQUAL -1)
							continue()
						endif()
						list(GET NBL_IMPL_SPLIT_ALL_RULE_OUTPUT_TARGETS ${NBL_IMPL_SPLIT_RULE_DEP_OUTPUT_INDEX} NBL_IMPL_SPLIT_DEP_TARGET)
						if(NOT "${NBL_IMPL_SPLIT_DEP_TARGET}" STREQUAL "${NBL_IMPL_SPLIT_GROUP_TARGET}")
							add_dependencies(${NBL_IMPL_SPLIT_GROUP_TARGET} ${NBL_IMPL_SPLIT_DEP_TARGET})
						endif()
					endforeach()
				endforeach()
			endforeach()
		...
		set(${IMPL_OUTPUT_VAR} ${ALL_OUTPUT_KEYS} PARENT_SCOPE)
		return()
	endif()

	macro(NBL_IMPL_CREATE_INTERNAL_RULE_TARGET TARGET_NAME)
		if(NOT TARGET ${TARGET_NAME})
			add_library(${TARGET_NAME} INTERFACE)
			set_target_properties(${TARGET_NAME} PROPERTIES VS_GLOBAL_BuildInParallel true)
		endif()
	endmacro()

	macro(NBL_IMPL_GET_RULE_GROUP_TARGET GROUP_NAME OUT_VAR)
		if("${GROUP_NAME}" STREQUAL "")
			set(${OUT_VAR} "${IMPL_TARGET}")
		else()
			list(FIND IMPL_RULE_GROUP_NAMES "${GROUP_NAME}" _NBL_IMPL_GROUP_INDEX)
			if(_NBL_IMPL_GROUP_INDEX EQUAL -1)
				string(MAKE_C_IDENTIFIER "${IMPL_TARGET}_${GROUP_NAME}" _NBL_IMPL_GROUP_IDENT)
				set(_NBL_IMPL_GROUP_TARGET "${IMPL_TARGET}__${_NBL_IMPL_GROUP_IDENT}")
				NBL_IMPL_CREATE_INTERNAL_RULE_TARGET(${_NBL_IMPL_GROUP_TARGET})
				add_dependencies(${IMPL_TARGET} ${_NBL_IMPL_GROUP_TARGET})
				list(APPEND IMPL_RULE_GROUP_NAMES "${GROUP_NAME}")
				list(APPEND IMPL_RULE_GROUP_TARGETS "${_NBL_IMPL_GROUP_TARGET}")
			else()
				list(GET IMPL_RULE_GROUP_TARGETS ${_NBL_IMPL_GROUP_INDEX} _NBL_IMPL_GROUP_TARGET)
			endif()
			set(${OUT_VAR} "${_NBL_IMPL_GROUP_TARGET}")
		endif()
	endmacro()

	macro(NBL_IMPL_GET_RULE_OWNER_TARGET GROUP_NAME BASE_KEY OUT_VAR)
		if("${GROUP_NAME}" STREQUAL "")
			set(${OUT_VAR} "${IMPL_TARGET}")
		else()
			NBL_IMPL_GET_RULE_GROUP_TARGET("${GROUP_NAME}" _NBL_IMPL_RULE_GROUP_TARGET)
			set(${OUT_VAR} "${_NBL_IMPL_RULE_GROUP_TARGET}")
		endif()
	endmacro()

	macro(NBL_IMPL_REGISTER_RULE_OUTPUT OUTPUT_PATH OWNER_TARGET)
		get_target_property(_NBL_IMPL_RULE_OUTPUTS ${IMPL_TARGET} NBL_IMPL_RULE_OUTPUTS)
		get_target_property(_NBL_IMPL_RULE_OUTPUT_TARGETS ${IMPL_TARGET} NBL_IMPL_RULE_OUTPUT_TARGETS)
		if(NOT _NBL_IMPL_RULE_OUTPUTS)
			set(_NBL_IMPL_RULE_OUTPUTS "")
		endif()
		if(NOT _NBL_IMPL_RULE_OUTPUT_TARGETS)
			set(_NBL_IMPL_RULE_OUTPUT_TARGETS "")
		endif()
		list(APPEND _NBL_IMPL_RULE_OUTPUTS "${OUTPUT_PATH}")
		list(APPEND _NBL_IMPL_RULE_OUTPUT_TARGETS "${OWNER_TARGET}")
		set_target_properties(${IMPL_TARGET} PROPERTIES
			NBL_IMPL_RULE_OUTPUTS "${_NBL_IMPL_RULE_OUTPUTS}"
			NBL_IMPL_RULE_OUTPUT_TARGETS "${_NBL_IMPL_RULE_OUTPUT_TARGETS}"
		)
	endmacro()

	macro(NBL_IMPL_SPLIT_RULE_DEPS OWNER_TARGET OUT_REMAINING_DEPS)
		set(_NBL_IMPL_REMAINING_RULE_DEPS "")
		get_target_property(_NBL_IMPL_RULE_OUTPUTS ${IMPL_TARGET} NBL_IMPL_RULE_OUTPUTS)
		get_target_property(_NBL_IMPL_RULE_OUTPUT_TARGETS ${IMPL_TARGET} NBL_IMPL_RULE_OUTPUT_TARGETS)
		if(NOT _NBL_IMPL_RULE_OUTPUTS)
			set(_NBL_IMPL_RULE_OUTPUTS "")
		...
		foreach(_NBL_IMPL_RULE_DEP IN LISTS DEPENDS_ON)
			list(FIND _NBL_IMPL_RULE_OUTPUTS "${_NBL_IMPL_RULE_DEP}" _NBL_IMPL_RULE_DEP_INDEX)
			if(NOT _NBL_IMPL_RULE_DEP_INDEX EQUAL -1)
				list(GET _NBL_IMPL_RULE_OUTPUT_TARGETS ${_NBL_IMPL_RULE_DEP_INDEX} _NBL_IMPL_DEP_TARGET)
				if(NOT "${_NBL_IMPL_DEP_TARGET}" STREQUAL "${OWNER_TARGET}")
					add_dependencies(${OWNER_TARGET} ${_NBL_IMPL_DEP_TARGET})
				endif()
			else()
				list(APPEND _NBL_IMPL_REMAINING_RULE_DEPS "${_NBL_IMPL_RULE_DEP}")
			endif()
		endforeach()
		set(${OUT_REMAINING_DEPS} "${_NBL_IMPL_REMAINING_RULE_DEPS}")
	endmacro()

	macro(NBL_IMPL_FILTER_OPTIONS_FOR_PREPROCESS OUT_VAR)
		set(_NBL_IMPL_FILTERED_OPTIONS "")
		set(_NBL_IMPL_SKIP_NEXT_OPTION FALSE)
		foreach(_NBL_IMPL_OPTION ${ARGN})
			if(_NBL_IMPL_SKIP_NEXT_OPTION)
				set(_NBL_IMPL_SKIP_NEXT_OPTION FALSE)
				continue()
			endif()
			if(_NBL_IMPL_OPTION STREQUAL "-T" OR _NBL_IMPL_OPTION STREQUAL "-E" OR _NBL_IMPL_OPTION STREQUAL "-Fo" OR _NBL_IMPL_OPTION STREQUAL "-Fc" OR _NBL_IMPL_OPTION STREQUAL "-MF")
				set(_NBL_IMPL_SKIP_NEXT_OPTION TRUE)
				continue()
			endif()
			if(_NBL_IMPL_OPTION MATCHES "^-T.+" OR _NBL_IMPL_OPTION MATCHES "^-E.+" OR _NBL_IMPL_OPTION MATCHES "^-Fo.+" OR _NBL_IMPL_OPTION MATCHES "^-Fc.+" OR _NBL_IMPL_OPTION MATCHES "^-MF.+" OR _NBL_IMPL_OPTION MATCHES "^-fspv-" OR _NBL_IMPL_OPTION MATCHES "^-fvk-" OR _NBL_IMPL_OPTION STREQUAL "-spirv" OR _NBL_IMPL_OPTION MATCHES "^\\$<\\$<CONFIG:(Debug|Release|RelWithDebInfo)>:-O[0-9]+>$")
				continue()
			endif()
			list(APPEND _NBL_IMPL_FILTERED_OPTIONS "${_NBL_IMPL_OPTION}")
		endforeach()
		set(${OUT_VAR} "${_NBL_IMPL_FILTERED_OPTIONS}")
	endmacro()
    ...
    foreach(INDEX RANGE ${LAST_INDEX})
		...
		string(JSON GROUP_TYPE ERROR_VARIABLE GROUP_ERROR TYPE "${IMPL_INPUTS}" ${INDEX} GROUP_BY)
		if(NOT GROUP_ERROR)
			if(NOT GROUP_TYPE STREQUAL "STRING")
				ERROR_WHILE_PARSING_ITEM("GROUP_BY must be a string when present")
			endif()
			string(JSON RULE_GROUP GET "${IMPL_INPUTS}" ${INDEX} GROUP_BY)
		endif()
		...
		NBL_IMPL_GET_RULE_OWNER_TARGET("${RULE_GROUP}" "${BASE_KEY}" NBL_NSC_RULE_OWNER_TARGET)

        set(OUTPUT_EXT ".spv")
        set(OUTPUT_PREFIX "$<CONFIG>/")
        set(OUTPUT_GROUP_BY_CONFIG TRUE)
        set(MODE_ARGS "")
		set(CONFIG_TARGET_INPUT "${TARGET_INPUT}")
		set(PREPROCESS_FRAGMENT_INCLUDE_FILE "")
        if(RULE_MODE STREQUAL "preprocess")
            set(OUTPUT_EXT ".preprocessed.hlsl")
            list(APPEND MODE_ARGS -P)
        endif()

		set(EFFECTIVE_REQUIRED_OPTIONS ${REQUIRED_COMMON_OPTIONS} ${REQUIRED_COMPILE_ONLY_OPTIONS})
		set(EFFECTIVE_RULE_COMPILE_OPTIONS ${COMPILE_OPTIONS})
		set(EFFECTIVE_COMMON_OPTIONS ${IMPL_COMMON_OPTIONS})
		if(RULE_MODE STREQUAL "preprocess")
			set(EFFECTIVE_REQUIRED_OPTIONS ${REQUIRED_COMMON_OPTIONS})
			NBL_IMPL_FILTER_OPTIONS_FOR_PREPROCESS(EFFECTIVE_RULE_COMPILE_OPTIONS ${COMPILE_OPTIONS})
			NBL_IMPL_FILTER_OPTIONS_FOR_PREPROCESS(EFFECTIVE_COMMON_OPTIONS ${IMPL_COMMON_OPTIONS})
		endif()
...
@RETVAL_EVAL@
		...
		function(GENERATE_KEYS PREFIX CAP_INDEX CAPS_EVAL_PART)
			...
			# generate .config file
				...
				set(CONFIG_TARGET_INPUT_FOR_RULE "${TARGET_INPUT}")
				if(RULE_MODE STREQUAL "preprocess")
					set(PREPROCESS_FRAGMENT_INCLUDE_FILE "${CONFIG_FILE}.include")
					string(CONFIGURE "${NBL_IMPL_NSC_PREPROCESS_INCLUDE_VIEW}" PREPROCESS_FRAGMENT_INCLUDE_CONTENT @ONLY)
					file(WRITE "${PREPROCESS_FRAGMENT_INCLUDE_FILE}" "${PREPROCESS_FRAGMENT_INCLUDE_CONTENT}")
					set(CONFIG_TARGET_INPUT_FOR_RULE "${PREPROCESS_FRAGMENT_INCLUDE_FILE}")
				endif()
				set(CAPS_EVAL "${CAPS_EVAL_PART}")
				set(CONFIG_TARGET_INPUT "${CONFIG_TARGET_INPUT_FOR_RULE}")
				...
				set(NBL_NSC_PREPROCESS_RAW_OUTPUT "")
				set(NBL_NSC_BUILD_STAMP_OUTPUT "")
				if(RULE_MODE STREQUAL "preprocess")
					set(NBL_NSC_PREPROCESS_RAW_OUTPUT "${TARGET_OUTPUT}.raw")
					set(NBL_NSC_BUILD_STAMP_OUTPUT "${TARGET_OUTPUT}.stamp")
				endif()
				...
				set(NBL_NSC_COMPILER_OUTPUT "${TARGET_OUTPUT}")
				if(RULE_MODE STREQUAL "preprocess")
					set(NBL_NSC_COMPILER_OUTPUT "${NBL_NSC_PREPROCESS_RAW_OUTPUT}")
				endif()
				...
				set(NBL_NSC_COMPILE_COMMAND
					...
					${NBL_NSC_EXTRA_ARGS}
					${MODE_ARGS}
					-Fc "${NBL_NSC_COMPILER_OUTPUT}"
					${EFFECTIVE_RULE_COMPILE_OPTIONS} ${EFFECTIVE_REQUIRED_OPTIONS} ${EFFECTIVE_COMMON_OPTIONS}
				...
				if(RULE_MODE STREQUAL "preprocess")
					list(APPEND NBL_NSC_BYPRODUCTS "${TARGET_OUTPUT}" "${NBL_NSC_PREPROCESS_RAW_OUTPUT}")
				endif()
				if(NBL_NSC_RULE_USE_DEPFILE)
					list(APPEND NBL_NSC_BYPRODUCTS "${DEPFILE_PATH}")
				endif()

				NBL_IMPL_SPLIT_RULE_DEPS("${NBL_NSC_RULE_OWNER_TARGET}" NBL_NSC_FILE_DEPENDS)
				set(NBL_NSC_CUSTOM_DEPENDS
					${NBL_NSC_FILE_DEPENDS}
					Nabla::nsc
					$<TARGET_NAME_IF_EXISTS:DeviceHeaders>
				)
				list(REMOVE_DUPLICATES NBL_NSC_CUSTOM_DEPENDS)

				set(NBL_NSC_CUSTOM_COMMAND_ARGS OUTPUT)
				if(RULE_MODE STREQUAL "preprocess")
					list(APPEND NBL_NSC_CUSTOM_COMMAND_ARGS "${NBL_NSC_BUILD_STAMP_OUTPUT}")
				else()
					list(APPEND NBL_NSC_CUSTOM_COMMAND_ARGS "${TARGET_OUTPUT}")
				endif()
				list(APPEND NBL_NSC_CUSTOM_COMMAND_ARGS
				...
				if(RULE_MODE STREQUAL "preprocess")
					list(APPEND NBL_NSC_CUSTOM_COMMAND_ARGS
						COMMAND "${CMAKE_COMMAND}"
							"-DINPUT=${NBL_NSC_PREPROCESS_RAW_OUTPUT}"
							"-DOUTPUT=${TARGET_OUTPUT}"
							-P "${NBL_IMPL_NSC_PREPROCESS_EXTRACT_SCRIPT}"
						COMMAND "${CMAKE_COMMAND}" -E touch "${NBL_NSC_BUILD_STAMP_OUTPUT}"
					)
				endif()
				if(NBL_NSC_RULE_USE_DEPFILE)
				...
				if(RULE_MODE STREQUAL "preprocess")
					list(APPEND NBL_NSC_OUT_FILES "${NBL_NSC_PREPROCESS_RAW_OUTPUT}" "${NBL_NSC_BUILD_STAMP_OUTPUT}")
				endif()
				if(NBL_NSC_RULE_USE_DEPFILE)
				...
				if(RULE_MODE STREQUAL "preprocess")
					list(REMOVE_ITEM HEADER_ONLY_LIKE "${TARGET_OUTPUT}")
					list(APPEND HEADER_ONLY_LIKE "${PREPROCESS_FRAGMENT_INCLUDE_FILE}")
				endif()
				set_source_files_properties(${HEADER_ONLY_LIKE} PROPERTIES 
					HEADER_FILE_ONLY ON
					VS_TOOL_OVERRIDE None
				)
				set(NBL_NSC_RULE_TARGET_SOURCES ${HEADER_ONLY_LIKE})
				if(RULE_MODE STREQUAL "preprocess")
					list(APPEND NBL_NSC_RULE_TARGET_SOURCES "${TARGET_OUTPUT}")
				endif()
				target_sources(${NBL_NSC_RULE_OWNER_TARGET} PRIVATE ${NBL_NSC_RULE_TARGET_SOURCES})
				if(CMAKE_CONFIGURATION_TYPES AND OUTPUT_GROUP_BY_CONFIG)
					foreach(_CFG IN LISTS CMAKE_CONFIGURATION_TYPES)
						...
						if(RULE_MODE STREQUAL "preprocess")
							list(APPEND NBL_NSC_OUT_FILES_IDE "${TARGET_OUTPUT_IDE}.raw" "${TARGET_OUTPUT_IDE}.stamp")
							set_source_files_properties("${TARGET_OUTPUT_IDE}" PROPERTIES
								HEADER_FILE_ONLY ON
								VS_TOOL_OVERRIDE None
							)
						endif()
				...
				NBL_IMPL_REGISTER_RULE_OUTPUT("${TARGET_OUTPUT}" "${NBL_NSC_RULE_OWNER_TARGET}")
